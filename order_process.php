...
<body>
  ...
  if (isset($_GET["option"]))
  {
    ...
    else if ($_GET["option"] == "fullview")
    {
      ...
      else if ($_GET["option"] == "start")//for  start work button
      {
        $get_orderid=$_GET["pk"];
        $sqlinsert="INSERT INTO order_process(order_id,actual_start_date,status)
        VALUES('".mysqli_real_escape_string($con,$get_orderid)."',
              '".mysqli_real_escape_string($con,date("Y-m-d"))."',
              '".mysqli_real_escape_string($con,"Progress")."')";
        $resultinsert=mysqli_query($con,$sqlinsert) or die ("sql error in sqlinsert" . mysqli_error($con));
        if($resultinsert)
        {
      ...
      else if ($_GET["option"] == "end")//for  end work button
      {
        $get_orderid=$_GET["pk"];
        $sqlupdate="UPDATE order_process SET
               actual_end_date='".mysqli_real_escape_string($con,date("Y-m-d"))."',
               status='".mysqli_real_escape_string($con,"Completed")."'
              WHERE order_id='".mysqli_real_escape_string($con,$get_orderid)."'";
        $resultupdate=mysqli_query($con,$sqlupdate) or die ("error in sqledit" . mysqli_error($con));
        if($resultupdate)
        {
          echo '<script> alert("Data stored successfully");
